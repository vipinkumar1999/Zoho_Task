...
float booking(struct booking *b,struct taxi t,int no)
{
...
// int i;
//FILE *fptr;
//fptr = fopen("output.txt","a");//Store output in output.txt
// if(taxi1_book>0)
// {
//  for(i=0;i<taxi1_book;i++)
//  {
//  fprintf(fptr,"     %d              %d         %s     %s       %.2f        %.2f      %d\n",b[1][i].booking_id,b[1][i].customer_id,b[1][i].from,b[1][i].to,b[1][i].pickuptime,b[1][i].droptime,b[1][i].amount);
//}
// }
// if(taxi2_book>0)
// {
...
{
//	FILE *fptr;
//	fptr = fopen("output.txt","w");//input is from input.txt
//	fprintf(fptr,"\n Booking_Id   Customer_Id    From  to  pickup_time   Drop_time   Amount\n");
//	fclose(fptr);
    ...
    while(1)
    {
        ...
        FILE *fptr;
        fptr = fopen("input.txt","r");//input is from input.txt
        ...
        if(choice==1)
        {
            ...
            for(i=0;i<count;i++)
            {
                fscanf(fptr,"%d %c %c %f",&t[i].customer_id, &t[i].pickup_point, &t[i].drop_point, &t[i].pickup_time);
